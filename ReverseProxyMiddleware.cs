        ...
        private readonly IEnumerable<string> IgnoreHeaders = new[]
{
      ...
      {
          Log.Warning("Ошибка выполнения запроса {proxy_url} через ip: {proxy_ip}, интервал ожидания для прокси увеличен, и составляет: {proxy_delay}, произвожу смену ip адреса, ", targetUri, proxyInfo?.IPAddress.ToString(), proxyInfo.Delay);
